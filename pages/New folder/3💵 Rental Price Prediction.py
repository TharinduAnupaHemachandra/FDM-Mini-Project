...
add_bg_from_local(r'F:\New FDM Project\pages\wp5.jpg')
...
menu_data = [
    {'icon': "🌃Most Demanding Property Type"},
    {'icon':"✒️Cancellation Policy Prediction"},
    {'icon': "💵 Rental Price Prediction"},
]
...
loaded_model = pickle.load(open(r'F:\ML Models\rfregressor.sav', 'rb'))
...
def price_prediction(input_data):
	...
	return "Predicted Price is $" + str(predicted_price)
...
def main():
	...
	st.markdown("<h1 style='text-align: center; color: white;'>Rental Price Prediction</h1>",
	            unsafe_allow_html = True)
	...
	cancellation_policy = st.radio(
		"Select the Cancellation Policy",
		('moderate', 'flexible', 'strict', 'no refunds', 'super strict'))
	...
	#amenities__Dryer = st.text_input('Availability of a Dryer')
	#amenities__Other_pet = st.text_input('Allowing to keep pets')
	amenities__Elevator_in_Building = st.radio('Availability of an elevator',
	                                           ('Available', 'Not Available'))
	if amenities__Elevator_in_Building == 'Available':
		amenities__Elevator_in_Building = 1
	else:
		amenities__Elevator_in_Building = 0

	amenities__Internet = st.radio('Internet facility',
	                              ('Available', 'Not Available'))
	if amenities__Internet == 'Available':
		amenities__Internet = 1
	else:
		amenities__Internet = 0

	amenities__Kitchen = st.radio('Availability of a kitchen',
	                                  ('Available', 'Not Available'))
	if amenities__Kitchen == 'Available':
		amenities__Kitchen = 1
	else:
		amenities__Kitchen = 0

	#amenities__Washer_and_Dryer = st.text_input('Availability of a washer and dryer')

	amenities__Air_Conditioning = st.radio('Air Conditioning Facility',
	                                           ('Available', 'Not Available'))
	if amenities__Air_Conditioning == 'Available':
		amenities__Air_Conditioning = 1
	else:
		amenities__Air_Conditioning = 0
	...
	if st.button('Predict the Price'):
		price = price_prediction(
			[accommodates, availability_30,
			 ...
			 guests_included, maximum_nights,
			 amenities__Elevator_in_Building,
			 amenities__Internet, amenities__Kitchen,
			 amenities__Air_Conditioning, bed_type_Airbed, bed_type_Couch,
			 bed_type_Futon, bed_type_Pull_out_Sofa, bed_type_Real_Bed,  cancellation_policy_flexible,
			 cancellation_policy_moderate, cancellation_policy_strict,
			 instant_bookable_f, instant_bookable_t, property_type_Apartment, property_type_Bed_Breakfast,property_type_Boat,property_type_Bungalow,
			 ...
			 property_type_Treehouse,property_type_Villa,room_type_Entire_home_apt,room_type_Private_room,room_type_Shared_room])
